...
namespace PSS_WINFORM_CONTROL.Views.itemView{
    public partial class itemView : XtraUserControl {
		...
		void InitBindings() {
		    var fluentAPI = mvvmContext.OfType<PSS_WINFORM_CONTROL.ViewModels.itemViewModel>();
			...
			fluentAPI.WithEvent<RowClickEventArgs>(order_productGridView, "RowClick")
						.EventToCommand(
						    x => x.itemorder_productDetails.Edit(null), x => x.itemorder_productDetails.SelectedEntity,
			...
			fluentAPI.SetBinding(order_productGridControl, g => g.DataSource, x => x.itemorder_productDetails.Entities);
														...
														fluentAPI.BindCommand(bbiorder_productNew, x => x.itemorder_productDetails.New());
																													fluentAPI.BindCommand(bbiorder_productEdit,x => x.itemorder_productDetails.Edit(null), x=>x.itemorder_productDetails.SelectedEntity);
																								fluentAPI.BindCommand(bbiorder_productDelete,x => x.itemorder_productDetails.Delete(null), x=>x.itemorder_productDetails.SelectedEntity);
